...
import { CONDITIONS, BUSINESS_INFO, OPENING_HOURS_SPECIFICATION, AREA_SERVED } from '../constants';
...
export const ConditionDetailPage: React.FC = () => {
    ...
    const relatedConditions = CONDITIONS.filter(c => c.slug !== slug);

    if (!condition) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-brand-bg">
                <div className="text-center">
                    <h1 className="text-4xl font-display text-brand-primary mb-4">Condition Not Found</h1>
        ...
        <>
            ...
            <section className="relative min-h-[60vh] flex items-end bg-brand-primary overflow-hidden">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full pb-16 pt-40">
                    <Link to="/#conditions" className="inline-flex items-center gap-2 text-white/60 hover:text-white transition-colors mb-8 group">
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-brand-muted">
                        <div className="flex items-center gap-2">
                            <img
                                ...
                                height={32}
                            />
                            <span>Medically reviewed by <Link to="/about" className="text-brand-primary font-semibold hover:text-brand-secondary transition-colors">Dr. Jason Bang, DC, FNP</Link></span>
                        </div>
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <RevealOnScroll animation="scale-up">
                        <div className="text-center max-w-2xl mx-auto mb-12">
                            <span className="text-brand-accent font-semibold tracking-widest uppercase text-xs mb-4 block">Recognize the Signs</span>
                            <h2 className="text-3xl md:text-4xl font-display text-brand-primary">Common Symptoms</h2>
                        </div>
                    </RevealOnScroll>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {condition.symptoms.map((symptom, idx) => (
                                <div key={idx} className="flex items-start gap-4 p-5 rounded-xl bg-brand-bg border border-brand-primary/5 hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-default">
                                    ...
                                    <span className="text-brand-text font-medium">{symptom}</span>
                                </div>
                            ))}
                        </div>
                    </RevealOnScroll>
                            ...
                            <RevealOnScroll animation="slide-right" delay={150}>
                                <div className="relative">
                                    ...
                                    <div className="relative aspect-[4/3] rounded-2xl overflow-hidden shadow-2xl">
                                        <img
                                            src={condition.image}
                                            alt={`Treatment for ${condition.title}`}
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                            {relatedConditions.map((related) => (
                                <div
                                    ...
                                    onClick={() => navigate(`/conditions/${related.slug}`)}
                                >
